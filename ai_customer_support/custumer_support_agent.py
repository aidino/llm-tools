...
class CustomerSupportAgent:
    ...
    def handle_query(self, query, user_id=None):
        try:
            ...
            context = "Relevant pass infomation: \n"
            if relevant_memories and "result" in relevant_memories:
                 for memory in relevant_memories["results"]:
                        if "memory" in memory:
                            context += f"- {memory['memory']}\n"
            
            # Generate a reponse using OpenAI
    ...
    def get_memories(self, user_id=None):
        try:
            ...
            st.error("Failed to retrieve memories: {e}")
            return None
...
# Add Button to genetate Synthetic data
